...
def gerar_danfce_pdf(xml_proc, extras=None):
    ...
    def _montar(altura_pagina):
        pdf = FPDF(orientation="P", unit="mm", format=(LARG, altura_pagina))
        ...
        # monta endereco evitando redundancia (cadastro as vezes poe nro/bairro no xLgr)
        partes_end = [emit_lgr.strip()]
        if emit_nro and emit_nro.upper() not in ("S/N", "SN", "") and emit_nro not in emit_lgr:
            partes_end.append(emit_nro)
        linha_end = ", ".join([p for p in partes_end if p])
        if emit_bairro and emit_bairro.upper() not in emit_lgr.upper():
            linha_end += f" - {emit_bairro}"
        if linha_end:
            C(linha_end, size=6)
        ...
        # ===== ITENS =====
        L("# Codigo Descricao Qtde Un Valor unit. Valor total", size=6, h=2.8)
        SEP()
    ...
    # passada 1: mede a altura real do conteudo
    _, y_final = _montar(600)
    altura = y_final + 5
    # passada 2: gera com a altura exata (sem sobra no rodape)
    pdf, _ = _montar(altura)
    out = pdf.output()
    return bytes(out)
